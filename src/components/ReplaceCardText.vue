...
const replaceCardText = computed(() =>
  props.text.replace(/\{\{(.+?)\}\}/g, (item) => {
    ...
    if (abilityImages.includes(ability.toLowerCase())) {
      ...
      return `<img src="https://res.cloudinary.com/datxpws7h/image/upload/v1776647713/images/abilities/${normalizedAbility}.png" class="abilityIcon" alt="${ability}"/>`
      ...
      return `<img src="https://res.cloudinary.com/datxpws7h/image/upload/v1776647713/images/stats/${imageType}/${number}.png" class="abilityIcon" alt="${ability}"/>`
    } else if (/^(\+|-)[1-9](health|strength|sun|brain)$/i.test(ability)) {
      const imageType = ability.replace(/(\+|-)\d/, '').toLowerCase()
      const subcategory = ability.includes('+') ? 'plus' : 'minus'
      const number = ability.match(/\d/)
      return `<img src="https://res.cloudinary.com/datxpws7h/image/upload/v1776647713/images/stats/${imageType}/${subcategory}/${number}.png" class="abilityIcon" alt="${ability}"/>`
    }
